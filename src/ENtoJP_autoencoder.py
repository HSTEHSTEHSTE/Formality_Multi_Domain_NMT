...
encoder = model.TransformerEncoder(en_dictionary_size, embed_dim, n_layers=1, device=device, pad_index=en_dict.pad(), dropout=.3).to(device=device)
decoder = model.TransformerDecoder(ja_dictionary_size, embed_dim, n_layers=1, device=device, pad_index=ja_dict.pad(), dropout=.4).to(device=device)
...
criterion = torch.nn.CrossEntropyLoss(ignore_index=ja_dict.pad(), label_smoothing=.3)
...
for iteration_number in range(0, max_iterations):
    ...
    memory, pad_mask = encoder(en_sentence_tensors)
    ...
    # dev forward pass
    loss = torch.tensor(0.).to(device=device)
    memory, pad_mask = encoder(dev_en_sentence_tensors)
...
for iteration_number in tqdm.tqdm(range(0, test_iterations), total=test_iterations):
    ...
    # test forward pass
    loss = torch.tensor(0.).to(device=device)
    memory, pad_mask = encoder(test_en_sentence_tensors)
    decoder_output = torch.tensor(ja_dict.bos()).unsqueeze(0).long().repeat(batch_size, 1).to(device=device) # (batch_size, 1)
    has_reached_eos = torch.ones([batch_size, 1]).to(device=device) # (batch_size, 1)
    eoses = torch.tensor(ja_dict.eos()).unsqueeze(0).long().repeat(batch_size, 1).to(device=device) # (batch_size, 1)
    for output_index in range(1, max_sentence_length - 1):
